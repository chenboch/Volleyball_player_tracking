...
def pose_points_yolo_old(detector,image,pose,tracker,args):
    timer_det.tic()
    ...
    # tracker.update()
    timer_track.toc()
    logger.info('TRACKING FPS --%s',1./max(timer_track.average_time,0.000001))
    device='cuda'
    ...
    # Somehow remove other objects from here
...
def pose_points_yolo(detector,image,pose):
    timer_det.tic()
    # starter, ender = torch.cuda.Event(enable_timing=True),   torch.cuda.Event(enable_timing=True)
    transform = TR.Compose([
        TR.ToPILImage(),
    ...
    detector.set_dataloaders([image])
    online_targets = detector.run_inference()
    # detections.save()
    timer_det.toc()
    logger.info('DET FPS -- %s',1./timer_det.average_time)
    # print(detections.shape)
    ...
    for t in online_targets:
        ...
        tid = t.track_id
        # vertical = tlwh[2] / tlwh[3] > args.aspect_ratio_threshs
        if tlwh[2] * tlwh[3] > 15 :#and not vertical:
            online_tlwhs.append(tlwh)
            online_ids.append(tid)
            online_scores.append(t.score)
    # tracker.update()
    ...
    # remove other objects from here? 
    nof_people = len(online_ids) if online_ids is not None else 0
    boxes = torch.empty((nof_people, 4), dtype=torch.int32,device= 'cuda')
    images = torch.empty((nof_people, 3, 256, 192))  # (height, width)
...
def get_pose_points(image, BBox, pose):
    device='cuda'
    
    # original bbox format: torch tensor, [x1, y1, x2, y2]
    # maybe I could modify to [x, y, w, h]
    image = image.copy()
    x, y, w, h = BBox
    image_crop = image[y:(y + h), x:(x + w), ::-1]
    correction_factor = 256 / 192 * w / h

    # only need to detect one person here
    transformed_image = torch.empty((1, 3, 256, 192))  # (height, width)

    if correction_factor > 1:
        # increase y side
        center = y + h // 2
        length = int(round(h * correction_factor))
        y1_new = int( center - length // 2)
        y2_new = int( center + length // 2)
        pad = (int(abs(y1_new-y))), int(abs(y2_new-(y + h)))
        image_crop = np.pad(image_crop,((pad), (0, 0), (0, 0)))
    
    elif correction_factor < 1:
        # increase x side
        center = x + w // 2
        length = int(round(w * 1 / correction_factor))
        x1_new = int( center - length // 2)
        x2_new = int( center + length // 2)
        pad = (abs(x1_new-x)), int(abs(x2_new-(x + w)))
        image_crop = np.pad(image_crop,((0, 0), (pad), (0, 0)))
    ...
    transformed_image[0] = transform(image_crop)
    ...
    pts[0, :, 0] = indicesc[0, :] * dim1 * h + y
    pts[0, :, 1] = indices[0, :] *dim2* w + x
    pts[0, :, 2] = c[0, :]
