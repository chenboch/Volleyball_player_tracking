...
if args.trt:
    ...
    # model = torch.hub.load('ultralytics/yolov5', 'yolov5n', pretrained=True)
    model = torch.hub.load('WongKinYiu/yolov7', 'custom','yolov7x.pt', trust_repo=True)
...
w, h = int(cv2.get(cv2.CAP_PROP_FRAME_WIDTH)),int(cv2.get(cv2.CAP_PROP_FRAME_HEIGHT))
if args.path_out is not None:
    
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    writer = cv2.VideoWriter(args.path_out, fourcc, cv2.get(cv2.CAP_PROP_FPS), (w, h))
frame_id = 0
timer = Timer()
timer_track = Timer()
timer_det = Timer()
...
if args.img is not None:
    frame_orig = cv2.imread("test.webp")
